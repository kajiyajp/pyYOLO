...
- [x] マウスドラッグで矩形描画
- [x] クラス選択（上部トグルボタン・F1〜・緑グロー）
- [x] クラスを classes.json で設定可能化（設定画面・再起動後反映）
- [x] YOLOフォーマット（.txt）で保存（Sキー）
- [x] アノテーション一覧表示・削除
- [x] 保存済みラベルの読み込み・再編集
- [x] 単一/複数画面（サムネイル）切替（Wキー）・注釈枠/未済表示
- [x] 単一画面の右上に画像番号オーバーレイ
- [x] ステータスバー・タブ別操作ガイド
- [x] ショートカット（Space/Ctrl+C/S/A/D/W/F1〜）
- [x] 実機UVCカメラ動作確認（Camo経由iPhoneで確認済み）
